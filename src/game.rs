...
fn handle_shop_state(shop_data: ShopData, action: &GameAction, effect: &GameEffect) -> GameState {
    match (action, effect) {
        (GameAction::PullOrb, GameEffect::HealthReachesZero) => todo!(),
        (GameAction::PullOrb, GameEffect::PointsLtMilestone) => todo!(),
        (GameAction::PullOrb, GameEffect::PointsGteMilestone) => todo!(),
...
fn handle_cashed_out_state(
    cashed_out_data: CashedOutData,
    action: &GameAction,
    effect: &GameEffect,
) -> GameState {
    match (action, effect) {
...
fn stf_game(state: GameState, action: &GameAction, effect: &GameEffect) -> GameState {
    match state {
        ...
        GameState::LevelComplete(level_complete_data) => {
            handle_level_complete_state(level_complete_data, action, effect)
        }
        GameState::Shop(shop_data) => handle_shop_state(shop_data, action, effect),
        GameState::CashedOut(cashed_out_data) => {
            handle_cashed_out_state(cashed_out_data, action, effect)
        }
        GameState::GameOver(game_over_data) => {
            handle_game_over_state(game_over_data, action, effect)
        }
    }
}
